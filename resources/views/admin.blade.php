...
@foreach($photo as $group_id => $amount)
	<div>{{$group_id}} <form method="post" action="/admin/createProduct"><input type="hidden" name="group_id" value="{{$group_id}}" ><input type="hidden" name="amount" value="{{$amount}}"><input type="submit" name="" value="创建"></form></div>
@endforeach
	</div>
